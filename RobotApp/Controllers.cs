...
namespace RobotApp
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void Background_KeyDown_1(object sender, Windows.UI.Xaml.Input.KeyRoutedEventArgs e)
        {
            ...
            if (e.Key.ToString() == "GamepadX") DCMotorCtrl.firing = true;
    ...
    public class Controllers
    {
        ...
        private static void Controller_ButtonPressed()
        {
            DCMotorCtrl.firing = true;
        }
        ...
        private static void Controller_DirectionChanged(ControllerVector sender)
        {
            ...
            DCMotorCtrl.speedValue = sender.Magnitude;
        }
        ...
        public static void SetRobotDirection(CtrlCmds cmd, int speed)
        {
            ...
            switch (cmd)
            {
                case CtrlCmds.Forward: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.ms2; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.ms1; break;
                case CtrlCmds.Backward: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop; break;
                case CtrlCmds.Left: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.ms1; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop; break;
                case CtrlCmds.Right: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.ms2; break;
                case CtrlCmds.ForwardLeft: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop; break;
                case CtrlCmds.ForwardRight: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.ms2; break;
                case CtrlCmds.BackLeft: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop; break;
                case CtrlCmds.BackRight: DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop; DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop; break;
                default:
                case CtrlCmds.Stop:         DCMotorCtrl.waitTimeLeft = DCMotorCtrl.PulseMs.stop;    DCMotorCtrl.waitTimeRight = DCMotorCtrl.PulseMs.stop;   break;
            }
            if (speed < (int)CtrlSpeeds.Min) speed = (int)CtrlSpeeds.Min;
            if (speed > (int)CtrlSpeeds.Max) speed = (int)CtrlSpeeds.Max;
            DCMotorCtrl.speedValue = speed;
        ...
        private static DCMotorCtrl.PulseMs lastWTL, lastWTR;
        private static int lastSpeed;
        static void dumpOnDiff(String title)
        {
            if ((lastWTR == DCMotorCtrl.waitTimeRight) && (lastWTL == DCMotorCtrl.waitTimeLeft) && (lastSpeed == DCMotorCtrl.speedValue)) return;
            Debug.WriteLine("Motors {0}: Left={1}, Right={2}, Speed={3}", title, DCMotorCtrl.waitTimeLeft, DCMotorCtrl.waitTimeRight, DCMotorCtrl.speedValue);
            lastWTL = DCMotorCtrl.waitTimeLeft;
            lastWTR = DCMotorCtrl.waitTimeRight;
            lastSpeed = DCMotorCtrl.speedValue;
        }
